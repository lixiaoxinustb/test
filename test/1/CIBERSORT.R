...
CoreAlg <- function(X, y, absolute, abs_method){
    ...
    if(Sys.info()['sysname'] == 'Windows') out <- mclapply(1:svn_itor, res, mc.cores=1) else ###mclapply是并行计算函数，在windows系统下无法开启多核并行计算，如果不是windows就可以
    out <- mclapply(1:svn_itor, res, mc.cores=svn_itor)  ##### 
    
    nusvm <- rep(0,svn_itor) ###重复函数第一个参数重复的内容，第二个参数重复的次数
...
CIBERSORT("E://cibersort//test//LM22.txt","E://cibersort//test//sampleall.txt",perm=100)
